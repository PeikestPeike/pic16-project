...
class MarkovText():
    '''
    This class expands on the original markov language model, generating fake texts with words as units.
    
    Attributes:
        text: string, the source text
        n: integer, the n-gram (a sequence of n words)
        length: integer, the expected length of the fake text, with words as units
        seed: string, the starting word of the fake text
    '''
    def __init__(self, text, n = 1, length = 30, seed = "Emma"):
        '''
    ...
    def make_a_sentence(self):
        ...
        Return:
            s: string, a sentence
        '''
        # get all ending punctuations
        sentenceEnders = re.compile('[.!?:;]')
        s = self.markov_text_word()
        # split the text to sentences and extract the first sentence
        s = sentenceEnders.split(s)[0] + "."
        return s
    
    def reading_ease_score(self):
        '''
        Print the Flesch reading ease score of the generated text and original text with the same length.
        Arg:
            None
        Return:
            None
        ...
        # compute the reading ease score of the fake text
        fake_text_score = round(reading_ease_score(fake_text), 2)
        
        # extract from the original text a text of the same length as the fake text
        start = random.randint(0, len(self.text) - len(fake_text))
        original_text = self.text[start:(start + len(fake_text))]
        # compute the reading ease score of the extracted original text 
